...
<%} %>
        <form action="../../commonauth" method="post" id="loginForm" class="form-horizontal" >
            ...
            <div class="row">
                    ...
                    %><div class="span2">
                       <a href="#"  class="main-link"><%=idpEntry.getKey()%></a>
                       <div class="slidePopper" style="display:none">
				<input type="text" id="domainName" name="domainName"/>
				<input type="button" class="btn btn-primary go-btn" onClick="javascript: myFunction('<%=idpEntry.getKey()%>','<%=idpEntry.getValue()%>','domainName')" value="Go" />
			</div>
		    </div>
                                <%}
                            }%>
	...
	<%
        if (idpAuthenticatorMapping != null && idpAuthenticatorMapping.size() != 0 &&
                (idpAuthenticatorMapping.size() != 1 && idpAuthenticatorMapping.containsKey(IdentityApplicationConstants.RESIDENT_IDP_RESERVED_NAME))) {
    	%>
  <div class="container">
	<div class="row">
		<div class="span12">
            ...
            <div class="row">
                    ...
                    %>
                              <div class="span2">
			                        ...
			                        <%}else{ %>
			                              <a href="javascript: handleNoDomain('<%=idpName%>','<%=idpEntry.getValue()%>')"  class="main-link"><%=idpName%></a>			                        
			                        <%} %>
		                      </div>
                            <%}else if(localAuthenticatorNames.size()>0 && localAuthenticatorNames.contains("IWAAuthenticator")) {
                            %>
                            	<div class="span2">
                                <a href="javascript: handleNoDomain('<%=idpEntry.getKey()%>','<%=idpEntry.getValue()%>')"  class="main-link">IWA</a>
	                            </div>
	                       <% 
                            }
